...
public class ScheduleAgainPolicy implements RejectedExecutionHandler {
	...
	/**
	 * If no worker thread is idle, caller is entering loop until a worker
	 * thread is released. The idle worker thread executes task r out of the
	 * caller's thread, unless the executor has been shut down, in which case
	...
	public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
		while (!executor.isShutdown() && executor.getQueue().size() >= workQueueSize) {
			try {
				...
				LOG.debug("Loop statistics;" + "sleepInterval=" + checkIdleWorkerInterval + "," + "poolSize=" + executor.getPoolSize() + "," + "activeCount=" + executor.getActiveCount() + ","
						+ "queueSize=" + executor.getQueue().size() + "," + "completedTaskCount=" + executor.getCompletedTaskCount() + "," + "taskCount=" + executor.getTaskCount() + ","
